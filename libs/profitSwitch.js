				...
				poloApi.getTicker(function(err, data){
					...
					Object.keys(symbolToAlgorithmMap).forEach(function(symbol){
						...
						if (data.hasOwnProperty('BTC_' + symbol)) {
							...
							marketData.BTC.ask = Number(btcData.lowestAsk);
							marketData.BTC.bid = Number(btcData.highestBid);
							marketData.BTC.last = Number(btcData.last);
							marketData.BTC.baseVolume = Number(btcData.baseVolume);
							marketData.BTC.quoteVolume = Number(btcData.quoteVolume);
						}
						if (data.hasOwnProperty('LTC_' + symbol)) {
							...
							marketData.LTC.ask = Number(ltcData.lowestAsk);
							marketData.LTC.bid = Number(ltcData.highestBid);
							marketData.LTC.last = Number(ltcData.last);
							marketData.LTC.baseVolume = Number(ltcData.baseVolume);
							marketData.LTC.quoteVolume = Number(ltcData.quoteVolume);
						}
						// save LTC to BTC exchange rate
						if (marketData.hasOwnProperty('LTC') && data.hasOwnProperty('BTC_LTC')) {
							var btcLtc = data.BTC_LTC;
							marketData.LTC.ltcToBtc = Number(btcLtc.highestBid);
						}
					});
		...
		poloApi.getOrderBook(symbolA, symbolB, function(err, data){
			...
			if (data.hasOwnProperty('bids')){
				data.bids.forEach(function(order){
					var price = Number(order[0]);
					var limit = Number(coinPrice * portalConfig.profitSwitch.depth);
					var qty = Number(order[1]);
			...
			if (totalQty > 0)
				marketData[symbolA].weightedBid = Number(depth / totalQty);
			callback();
		});
	};
				...
				cryptsyApi.getTicker(function(err, data){
					...
					Object.keys(symbolToAlgorithmMap).forEach(function(symbol){
						...
						if (results && results.hasOwnProperty(symbol + '/BTC')) {
							...
							marketData.BTC.last = Number(btcData.lasttradeprice);
							marketData.BTC.baseVolume = Number(marketData.BTC.last / btcData.volume);
							marketData.BTC.quoteVolume = Number(btcData.volume);
							if (btcData.sellorders !== null)
								marketData.BTC.ask = Number(btcData.sellorders[0].price);
							if (btcData.buyorders !== null) {
								marketData.BTC.bid = Number(btcData.buyorders[0].price);
								var limit = Number(marketData.BTC.bid * portalConfig.profitSwitch.depth);
								var depth = Number(0);
								var totalQty = Number(0);
							   ...
							   if (totalQty > 0)
								   marketData.BTC.weightedBid = Number(depth / totalQty);
						   }
						}
						...
						if (results && results.hasOwnProperty(symbol + '/LTC')) {
							...
							marketData.LTC.last = Number(ltcData.lasttradeprice);
							marketData.LTC.baseVolume = Number(marketData.LTC.last / ltcData.volume);
							marketData.LTC.quoteVolume = Number(ltcData.volume);
							if (ltcData.sellorders !== null)
								marketData.LTC.ask = Number(ltcData.sellorders[0].price);
							if (ltcData.buyorders !== null) {
								marketData.LTC.bid = Number(ltcData.buyorders[0].price);
								var limit = Number(marketData.LTC.bid * portalConfig.profitSwitch.depth);
								var depth = Number(0);
								var totalQty = Number(0);
							   ...
							   if (totalQty > 0)
								   marketData.LTC.weightedBid = Number(depth / totalQty);
						   }
						}
					});
					taskCallback();
					...
					Object.keys(symbolToAlgorithmMap).forEach(function(symbol){
						response.data.forEach(function(market){
							...
							if (market.exchange == 'BTC' && market.code == symbol) {
								...
								marketData.BTC.last = Number(market.last_price);
								marketData.BTC.baseVolume = Number(market.Bittrex);
								marketData.BTC.quoteVolume = Number(market.Bittrex / market.last_price);
								marketData.BTC.ask = Number(market.top_ask);
								marketData.BTC.bid = Number(market.top_bid);
							}
							...
							if (market.exchange == 'LTC' && market.code == symbol) {
								...
								marketData.LTC.last = Number(market.last_price);
								marketData.LTC.baseVolume = Number(market.Bittrex);
								marketData.LTC.quoteVolume = Number(market.Bittrex / market.last_price);
								marketData.LTC.ask = Number(market.top_ask);
								marketData.LTC.bid = Number(market.top_bid);
							}
						});
					});
					taskCallback();
					...
					Object.keys(symbolToAlgorithmMap).forEach(function(symbol){
						response.result.forEach(function(market){
							...
							if (market.exchange == 'BTC' && market.code == symbol) {
								...
								marketData.BTC.last = Number(market.Last);
								marketData.BTC.baseVolume = Number(market.BaseVolume);
								marketData.BTC.quoteVolume = Number(market.BaseVolume / market.Last);
								marketData.BTC.ask = Number(market.Ask);
								marketData.BTC.bid = Number(market.Bid);
							}
							...
							if (market.exchange == 'LTC' && market.code == symbol) {
								...
								marketData.LTC.last = Number(market.Last);
								marketData.LTC.baseVolume = Number(market.BaseVolume);
								marketData.LTC.quoteVolume = Number(market.BaseVolume / market.Last);
								marketData.LTC.ask = Number(market.Ask);
								marketData.LTC.bid = Number(market.Bid);
							}
						});
					});
					taskCallback();
